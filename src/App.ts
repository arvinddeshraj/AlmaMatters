...
import startSocket from './chatServer';
...
export default class App {
   ...
   * @param {string} name
   */
  constructor(credentials: ServerOptions = null, name: string = 'Alma Matters') {
    this.app = express();
    ...
    this.app.use(async (req, res, next) => {
      if ( req.url == '/api/auth/getToken' || req.url == '/api/query' ) {
        next();
      } else {
        if (req.get('x-auth')) {
